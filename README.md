...
| 교사 | 학생 일괄 생성 + **전체 명단에서 골라 담기**, 교과·단원·활동 관리, **진도 현황**(반×활동 완료율), **서술 읽기**, 기록 CSV |
...
그다음 **`0012` → `0013` → `0014` → `0015` → `0016` 을 순서대로** 이어서 실행하세요.
...
>
> `0016` 은 지금의 `profiles.teacher_id` 담당 관계를 `teacher_students` 로 **그대로 옮깁니다** —
> 실행 직후 각 교사의 목록은 전과 같습니다. `teacher_id` 는 남지만 뜻이 **"이 계정을 만든 사람"**
> 으로 바뀌어, 이제 **계정 삭제 권한**을 가리는 데만 쓰입니다.
...
**여러 교사가 함께 쓸 때** — 학생 계정은 **한 번만 만들고 나눠 담습니다.**
학생 관리 화면의 **전체 학생에서 담아 오기**에 이 서버의 학생이 학년·반·번호 순으로 모두 나오니,
누가 만들었든 골라서 내 목록에 담으면 됩니다. **한 학생을 여러 선생님이 함께 담아도 됩니다**
(담임 + 교과 교사). 내 목록에서 빼도 계정과 기록은 남습니다.
교과·단원·활동은 **공유**되므로(학년이 같으면), 자료도 학생도 함께 쓰는 구조입니다.
관리자는 전체 학생과 담당 교사들을 볼 수 있습니다.

> **계정 삭제**는 그 계정을 **만든 교사(또는 관리자)만** 할 수 있습니다 —
> 함께 담은 다른 선생님의 학생까지 사라지는 일을 막기 위해서입니다.
> 비밀번호 재설정은 **담은 교사면 누구나** 할 수 있습니다(수업 중에 바로 필요하므로).
...
학생 프로필(`profiles`)은 학년·반·번호를 가지고, **담당 교사는 `teacher_students`**(교사↔학생, 다대다)가 잇습니다.
`profiles.teacher_id` 는 **그 계정을 만든 사람**이며 계정 삭제 권한에만 쓰입니다.
...
- 교사는 **자기 목록에 담은 학생만** 조회·수정할 수 있습니다(RLS + 서버 API 이중 확인).
  `service_role` 은 RLS를 우회하므로, 서버 코드에서 소유권을 따로 검사합니다.
- 예외는 **전체 학생 명단**입니다. 담지 않은 학생도 골라 담을 수 있어야 하므로
  `all_students()` RPC 가 교사·관리자에게 학년·반·번호·이름까지만 내려 줍니다
  (비밀번호 상태나 활동 기록은 담기지 않습니다).
- **계정 삭제**는 그 계정을 만든 교사와 관리자만 할 수 있습니다 —
  한 학생을 여러 교사가 담을 수 있으므로, 담았다는 사실만으로는 삭제 근거가 되지 않습니다.
